<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Budal Kirim</title>
    <link rel="stylesheet" href="style.css">
</head>

<body>
    <header>
        <nav>
            <ul class="seger">
                <ul class="biasa">
                    <li><a href="landing page.php">Beranda</a></li>
                    <li><a href="tentang kami.php">Tentang Kami</a></li>
                    <li><a href="Layanan.php">Layanan</a></li>
                </ul>
                <ul class="RS">
                    <a class="sakit" href="start.php" class="logo"><span class="sehat">Budal</span> Kirim</a>
                </ul>
            </ul>
        </nav>
    </header>
    <section>
        <div class="cover">
            <div>
                <h2 class="budal">
                    <p class="sakit" href="#" class="logo"><span class="sehat">Budal</span> Kirim</p>
                </h2>
                <p class="paragraf">Budal kirim adalah jasa pengiriman barang di seluruh Indonesia, kami mencangkup seluruh wilayah di seluruh Indonesia dari perkotaan hingga pelosok pelosok terpencil di Indonesia. Kami menjalankan jasa pengiriman barang dengan cepat, murah,
                    dan bisa COD (Cash on Delivery), kami melayani pengiriman antar negara, antar provinsi, antar kota, antar kecamatan, antar desa, maupun antar tetangga. Kami memiliki keunggulan pengiriman yang cepat, daerah jangkauan yang luas, dan
                    keramahan kurir kami.
                </p>

            </div>
            <div>
                <img class="gambar" src="kurir.jpeg">
            </div>
        </div>
        <div class="alhamdulillah">
            <a class="kontol" href="kirim saiki.php" class="allahuakbar"><span class="allah">Budal</span> Ngirim Saiki</a>
        </div>
    </section>


    <footer class="anjay">
        <div class="footer-content">
            <div class="footer-section about">
                <h2 class="Aboutus">About Us</h2>
                <p class="tentangkita">Budal kirim adalah jasa pengiriman barang di seluruh Indonesia, kami mencangkup seluruh wilayah di seluruh Indonesia dari perkotaan hingga pelosok pelosok terpencil di Indonesia. Kami menjalankan jasa pengiriman barang dengan cepat, murah,
                    dan bisa COD (Cash on Delivery), kami melayani pengiriman antar negara, antar provinsi, antar kota, antar kecamatan, antar desa, maupun antar tetangga. Kami memiliki keunggulan pengiriman yang cepat, daerah jangkauan yang luas, dan
                    keramahan kurir kami.</p>
            </div>
            <div class="footer-section contact">
                <h2>Hubungi Kami</h2>
                <p>Email: info@pengirimanbarang.com<br> Telepon: (123) 456-7890</p>
            </div>
            <div class="footer-section social">
                <h2>Ikuti Kami</h2>
                <p>Temui kami di media sosial:</p>
                <a href="Profile.php" target="_blank"><img src="prsn icon.png" alt="Profile"></a>
                <a href="https://twitter.com/deianersait" target="_blank"><img src="twt icon.png" alt="Twitter"></a>
                <a href="https://www.instagram.com/reycannavaro/" target="_blank"><img src="ig icon.png" alt="Instagram"></a>
            </div>
        </div>
        <div class="footer-bottom">
            <p>&copy; 2023 Jasa Pengiriman Barang - All rights reserved.</p>
        </div>
    </footer>

</body>

</html>